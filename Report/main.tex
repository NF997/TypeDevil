...
Consider listing \ref{listingRunningExample}, which shows the running example used to introduce the various steps of TypeDevils pipeline.
...
Nevertheless, the only pruning technique which utilizes a static analysis which takes place before the main dynamic analysis of TypeDevil should be described in the following: Pruning by belief analysis.\\
...
\cite{DBLP:conf/icse/TanXCLYS17} also implemented a integrated type analysis which first analyzes programs statically and afterwards refines the results with a subsequent dynamic analysis which implements the basic ideas of TypeDevil.
